class Staff::EntriesForm
  ...
  def save
    ...
    not_approved_entry_id = @not_approved.split(':').map(&:to_i)
    canceled_entry_ids = @canceled.split(':').map(&:to_i)
    not_canceled_entry_id = @not_canceled.split(':').map(&:to_i)

    ActiveRecord::Base.transaction do
      @program.entries.where(id: approved_entry_ids).update_all(approved: true)
      @program.entries.where(id: not_approved_entry_id).update_all(approved: false)
      @program.entries.where(id: canceled_entry_ids).update_all(canceled: true)
      @program.entries.where(id: not_canceled_entry_id).update_all(canceled: false)
    end
  end
end